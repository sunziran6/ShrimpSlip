    ...
    <header class="fixed top-0 left-0 right-0 z-50 h-16 bg-secondary border-b border-border">
      <div class="h-full max-w-[1400px] mx-auto px-6 flex items-center justify-between">
        ...
        <div class="flex items-center gap-1">
          ...
          <!-- Admin Badge -->
          <span class="px-2 py-0.5 rounded-md bg-primary/15 text-primary text-xs font-semibold border border-primary/25 mr-3">
            管理员版
          </span>
...
import { DataAnalysis, Goods, Coin, SwitchButton } from '@element-plus/icons-vue'

const router = useRouter()

const navItems = [
  { path: '/admin', label: '使用统计', icon: DataAnalysis },
  { path: '/admin/products', label: '商品管理', icon: Goods },
  { path: '/admin/revenue', label: '收益统计', icon: Coin },
]
